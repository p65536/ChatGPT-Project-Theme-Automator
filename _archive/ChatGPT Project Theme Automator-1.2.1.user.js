...
(() => {
    ...
    SELECTORS.USER_TEXT_CONTENT_CSS_TARGET = `${SELECTORS.USER_MESSAGE} .${SELECTORS.CLASS_WHITESPACE_PRE_WRAP}`;
    SELECTORS.ASSISTANT_MARKDOWN_CSS_TARGET = `${SELECTORS.ASSISTANT_MESSAGE} .${SELECTORS.CLASS_MARKDOWN}`;
    SELECTORS.ASSISTANT_WHITESPACE_CSS_TARGET = `${SELECTORS.ASSISTANT_MESSAGE} .${SELECTORS.CLASS_WHITESPACE_PRE_WRAP}`;
    ...
    const STYLE_RULE_MAP = {
        user: {
            ...
            bubbleBackgroundColor: {
                selector: `${SELECTORS.USER_MESSAGE} ${SELECTORS.RAW_USER_BUBBLE}`,
        ...
        assistant: {
            ...
            bubbleBackgroundColor: {
                selector: [`${SELECTORS.ASSISTANT_MESSAGE} ${SELECTORS.RAW_ASSISTANT_MD_BUBBLE}`, `${SELECTORS.ASSISTANT_MESSAGE} ${SELECTORS.RAW_ASSISTANT_PRE_BUBBLE}`],
                property: 'background-color',
    ...
    class ThemeManager {
        ...
        createThemeCSSTemplate() {
            return `
                ...
                ${SELECTORS.ASSISTANT_MESSAGE} div[class*="tableContainer"],
                ${SELECTORS.ASSISTANT_MESSAGE} div[class*="tableWrapper"] {
                    width: auto;
                    overflow-x: auto;
                    box-sizing: border-box;
                    display: block;
    ...
    class BubbleNavigationManager {
        ...
        injectBubbleNavigationStyle() {
            ...
            style.textContent = `
                ...
                ${SELECTORS.ASSISTANT_MESSAGE} .cpta-bubble-nav-container {
                    left: -25px;
                }
                ${SELECTORS.USER_MESSAGE} .cpta-bubble-nav-container {
                    right: -25px;
                }
                .cpta-nav-group-top {
                    position: absolute;
        ...
        _shouldShowScrollTop(turnNode, messageElement) {
            ...
            const threshold = config.options.icon_size * multiplier;
            const assistantMessages = Array.from(turnNode.querySelectorAll(SELECTORS.ASSISTANT_MESSAGE));
    ...
    class ObserverManager {
        ...
        scanForExistingTurns() {
            ...
            if (existingTurnNodes.length > 0) {
                for (const turnNode of existingTurnNodes) {
                    // Ensure we don't attach a second observer to an already observed turn
        ...
        _processTurnSingle(turnNode) {
            ...
            const turnObserver = new MutationObserver((mutations, observer) => {
                ...
                // --- Completion Check ---
                const userMessageElement = turnNode.querySelector(SELECTORS.USER_MESSAGE);
                const assistantMessageElement = turnNode.querySelector(SELECTORS.ASSISTANT_MESSAGE);
